#' Generate landscape level covariate rasters for sample plan.
#'
#' Landscape covariates are based on a 25m DEM and uses SAGA GIS and additonal tools to
#' generate the required outputs for the conditional Latin Hyper Cube Sample
#'
#' Depending on your system the path to `saga_cmd` may need to be specified.
#'
#' @param dtm An `SpatRast` object or path to a spatial file (.tif) which contains the elevation data.
#'      This may be a digital terain model (dtm) or digital elevation model (dem). One possible usually generated
#'      when calling [create_template_raster()]. Should be a meter based coordinate reference system.
#' is a 25m dtm raster object ideally cropped to the AOI watershed boundaries
